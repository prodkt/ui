...
import {
  ...
  CodeBlockCopyButton,
  CodeBlockContent,
} from '@/components/ui/prodkt-ui/code-block';
...
const Example = () => (
  <div className="p-4">
    <CodeBlock defaultValue={code[0].language}>
      ...
      {code.map((item) => (
        <CodeBlockContent key={item.language} value={item.language} syntaxHighlighting={false}>
          {item.code}
        </CodeBlockContent>
      ))}
    </CodeBlock>
...
const Example = () => (
  <div className="p-4">
    <CodeBlock defaultValue={code[0].language}>
      ...
      {code.map((item) => (
        <CodeBlockContent key={item.language} value={item.language} themes={{
          light: 'github-light',
